...
export async function getAllProducts(): Promise<Product[]> {
  ...
  if (!data) {
    data = productsData;
    console.log('📦 Utilisation de products-full.json (fallback)');
  }
  ...
  return productsArray.map((p: any) => {
    ...
    if (p.images && Array.isArray(p.images) && p.images.length > 0) {
      ...
      if (validImages.length === 0) {
        validImages = validImage !== '/placeholder-image.svg' ? [validImage] : ['/placeholder-image.svg'];
      }
    } else {
    ...
    return {
      ...
      // Utiliser les vraies images des produits ou placeholder
      image_url: validImage,
      image: validImage,
      // S'assurer que images est un tableau avec les images valides
      images: validImages.length > 0 ? validImages : ['/placeholder-image.svg'],
      // Mapper les attributs si présents (nouveau format)
      attributes: p.attributes || (p.stone || p.material || p.dimensions || p.origin ? {
        stone: p.stone,
        material: p.material,
        dimensions: p.dimensions,
        origin: p.origin,
      } : undefined),
      // Compatibilité avec les anciens champs (pour filtrage)
...
export async function searchProducts(query: string): Promise<Product[]> {
  ...
  return allProducts.filter(p => {
    ...
    const categoryMatch = p.category?.toLowerCase().includes(lowerQuery) || false;
    const descMatch = p.description && typeof p.description === 'string' && p.description.toLowerCase().includes(lowerQuery);
    
    // Priorité au Nom et à la Catégorie
 ...
 */
export async function getProductBySlugFromJSON(slug: string): Promise<Product | null> {
...
export async function getPaginatedProducts(
  ...
  const currentPage = Math.max(1, Math.min(page, totalPages)); // Clamp entre 1 et totalPages
